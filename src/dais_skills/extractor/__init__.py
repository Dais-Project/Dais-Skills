...
from .resource import SkillResource, create_from_bytes
...
class Skill(SkillMd):
    ...
    def from_zip(cls, zip_file: zipfile.ZipFile) -> "Skill":
        paths = [ZipPath(name) for name in zip_file.namelist()]
        ...
        for info in zip_file.infolist():
            ...
            if info_path == skill_md:
                continue
            if skill_root != ZipPath(".") and skill_root not in info_path.parents:
                continue

            relative = info_path.relative_to(skill_root)
            content_bytes = zip_file.read(info.filename)
            resources.append(create_from_bytes(str(relative), content_bytes))

        return cls(**skill.__dict__, resources=resources)
